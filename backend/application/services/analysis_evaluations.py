...
class AnalysisEvaluationApplicationService:
        ...
        with bound_log_context(job_id=str(job_id)):
            ...
            for mode in payload.modes:
                ...
                if (
                    ...
                    and existing.status != EvaluationStatus.FAILED.value
                ):
                ...
                if snapshot is None:
                    if analysis_result is None:
                        raise ConflictAppError(
                            "Analysis must be completed before LLM evaluation can run."
                        )
            ...
            refreshed_records = await self._get_requested_records(
                job_id=job_id, modes=payload.modes
            )
            return EvaluationListResponse(
                items=[self._to_read(record) for record in refreshed_records]
            )
    ...
    async def process_evaluation(
        self, *, job_id: UUID, mode: EvaluationMode
    ) -> EvaluationRecordRead | None:
        with bound_log_context(job_id=str(job_id)):
            ...
            try:
                response = await self._get_engine().evaluate(
                    EvaluationRequest(mode=mode, context=context)
                )
                refreshed_record = await self.evaluations.get_for_job_and_mode(
                ...
                completed_metadata = self._merge_evaluation_metadata(
                    metadata_json={
                        **(refreshed_record.metadata_json or {}),
                        **(response.telemetry or {}),
                    },
                    stage="completed",
                ...
                completed_record = await self.evaluations.get_for_job_and_mode(
                    job_id=job_id, mode=mode
                )
                ...
                if refreshed_record is not None:
                    ...
                    await self.session.commit()
                    await self._refresh_job_progress_after_evaluation(job)
                    failed_record = await self.evaluations.get_for_job_and_mode(
                        job_id=job_id, mode=mode
                    )
    ...
    async def _get_requested_records(
        ...
        *,
        ...
        return [record_by_mode[mode.value] for mode in modes if mode.value in record_by_mode]
    ...
    def _is_stale_processing_record(self, record: LLMEvaluationRecord) -> bool:
        if record.status != EvaluationStatus.PROCESSING.value:
            return False
        return record.updated_at < datetime.now(UTC) - timedelta(
            seconds=settings.llm_processing_stale_after_seconds
        )

    async def _expire_stale_processing_record(
        self, record: LLMEvaluationRecord
    ) -> LLMEvaluationRecord:
        ...
        if job is not None:
            await self._refresh_job_progress_after_evaluation(job)
        refreshed_record = await self.evaluations.get_for_job_and_mode(
            job_id=record.job_id, mode=record.mode
        )
        ...
        *,
        ...
        metadata["progress"] = {
            **progress,
            ...
            "sections_ready": list(
                sections_ready if sections_ready is not None else progress.get("sections_ready", [])
            ),
            "expected_sections": list(
                progress.get("expected_sections", self.EVALUATION_EXPECTED_SECTIONS)
            ),
    ...
    async def _publish_job_progress(
        ...
        *,
        ...
        status = (
            ...
            else ("completed" if job.status == JobStatus.SUCCEEDED else job.status.value)
        )
    ...
    async def _refresh_job_progress_after_evaluation(self, job: InferenceJob) -> None:
        ...
        active_processing = next(
            (record for record in records if record.status == EvaluationStatus.PROCESSING.value),
            None,
        )
        active_queued = next(
            (record for record in records if record.status == EvaluationStatus.QUEUED.value), None
        )
