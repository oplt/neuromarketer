from backend.db.models import CollaborationReview, OrganizationSsoConfig, UserMfaCredential, WorkspaceInvite


def test_auth_related_enums_bind_lowercase_database_values() -> None:
    assert WorkspaceInvite.__table__.c.status.type.enums == ["pending", "accepted", "revoked", "expired"]
...
def test_collaboration_enums_bind_lowercase_database_values() -> None:
    assert CollaborationReview.__table__.c.entity_type.type.enums == ["analysis_job", "analysis_comparison"]
